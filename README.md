...
<body class="bg-slate-100 font-sans text-gray-800">
            ...
            <div id="tab-admin" class="hidden-section space-y-6">
                <div class="bg-white p-6 rounded-lg shadow-md border-t-4 border-emerald-600">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-sm">Recipient Unique User ID (e.g. DF43H) or Mobile No:</label>
    ...
    <!-- JavaScript Logic -->
    <script>
        const DB_KEY = "cricket_portal_db_v6";
        ...
        function getDB() {
            ...
            if (!data) {
                let initial = {
                    users: {},       // key: phone number -> { phone, userid, wallet }
                    userIDs: {},     // key: 5-char userid (lowercase) -> { phone, wallet }
                    devices: {},
                    matches: [],
                    pointsTable: {}, 
        ...
        function handleLogin() {
            ...
            // Check if this userid is already tied to a DIFFERENT phone number
            if (db.userIDs[userid] && db.userIDs[userid].phone !== phone) {
            ...
            // Wallet amount calculation: Check if userid or phone has pre-assigned points from admin
            let walletBalance = 50;
            if (phone === "9569981484") {
                walletBalance = 5000;
            }

            // Agar admin ne pehle hi is userid (jaise df43h) par points bhej rakhe hain
        ...
        function initDashboard() {
            ...
            // Sync latest wallet from userIDs mapping just in case admin sent points while logged in
        ...
        function checkRecipientUser() {
            ...
            if (foundWallet !== null) {
                ...
                statusSpan.innerText = `ℹ️ ID/Phone not logged in yet. Points will be saved safely for this code and credited when they log in!`;
            }
        }
        ...
        function adminSendPoints() {
            ...
            // Check if input is a 5-char user ID or a phone number
            let targetUserId = null;
            let targetPhone = null;

            if (inputVal.length === 5) {
                targetUserId = inputVal;
                if (!db.userIDs[targetUserId]) {
                    db.userIDs[targetUserId] = { phone: "Pending Login", wallet: 0 };
                }
                db.userIDs[targetUserId].wallet += amount;
                
                // Agar ye user kisi phone ke sath pehle se linked hai toh wahan bhi update karo
                let linkedPhone = db.userIDs[targetUserId].phone;
                if (linkedPhone && db.users[linkedPhone]) {
                    db.users[linkedPhone].wallet = db.userIDs[targetUserId].wallet;
                }
            } else {
                // Phone number ke through bheja gaya ho
                targetPhone = inputVal;
                if (!db.users[targetPhone]) {
                    db.users[targetPhone] = { phone: targetPhone, userid: targetPhone, wallet: 0 };
                }
                db.users[targetPhone].wallet += amount;
            }

            saveDB(db);
            
            let totalWalletDisplay = targetUserId ? db.userIDs[targetUserId].wallet : db.users[targetPhone].wallet;
            alert(`Successfully sent ${amount} points! Total Wallet Balance for [${inputVal.toUpperCase()}]: ${totalWalletDisplay} Pts`);
